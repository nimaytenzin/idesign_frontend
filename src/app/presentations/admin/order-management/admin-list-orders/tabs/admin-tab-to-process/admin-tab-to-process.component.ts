...
@Component({
	selector: 'app-admin-tab-to-process',
	standalone: true,
	imports: [CommonModule, FormsModule, TableModule, TagModule],
	templateUrl: './admin-tab-to-process.component.html',
})
export class AdminTabToProcessComponent implements OnInit, OnChanges {
	...
	loadOrders(): void {
		...
		this.orderService.getOrdersToConfirm().subscribe({
			next: (data: Order[]) => {
				this.orders = data ?? [];
				this.loading = false;
				this.cdr.markForCheck();
			...
			error: () => {
				this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to load orders to process' });
				this.loading = false;
				this.cdr.markForCheck();
			},
		});
