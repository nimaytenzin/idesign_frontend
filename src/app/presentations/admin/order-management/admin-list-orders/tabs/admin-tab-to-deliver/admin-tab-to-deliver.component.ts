...
@Component({
	selector: 'app-admin-tab-to-deliver',
	standalone: true,
	imports: [CommonModule, FormsModule, TableModule, TagModule],
	templateUrl: './admin-tab-to-deliver.component.html',
})
export class AdminTabToDeliverComponent implements OnInit, OnChanges {
	...
	loadOrders(): void {
		...
		this.orderService.getOrdersToDeliver().subscribe({
			...
			error: () => {
				this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to load orders to deliver' });
				this.loading = false;
				this.cdr.markForCheck();
			},
		});
