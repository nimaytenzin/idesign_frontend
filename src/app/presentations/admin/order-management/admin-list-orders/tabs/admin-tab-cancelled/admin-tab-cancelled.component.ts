...
@Component({
	selector: 'app-admin-tab-cancelled',
	standalone: true,
	imports: [CommonModule, FormsModule, TableModule, TagModule, ButtonModule, CalendarModule, PaginatorModule],
	templateUrl: './admin-tab-cancelled.component.html',
})
export class AdminTabCancelledComponent implements OnInit, OnChanges {
	...
	loadOrders(): void {
		...
		this.orderService.getOrdersAdminCancelled({
			page: this.pagination.page,
			limit: this.pagination.limit,
			updatedAtFrom: this.toIsoDateString(this.historyDateFrom),
			updatedAtTo: this.toIsoDateString(this.historyDateTo),
		}).subscribe({
			next: (data) => {
			...
			error: () => {
				this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to load cancelled orders' });
				this.loading = false;
				this.cdr.markForCheck();
			},
		});
