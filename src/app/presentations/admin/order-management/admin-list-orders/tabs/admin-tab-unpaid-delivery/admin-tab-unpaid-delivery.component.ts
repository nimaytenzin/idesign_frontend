...
@Component({
	selector: 'app-admin-tab-unpaid-delivery',
	standalone: true,
	imports: [CommonModule, FormsModule, TableModule, TagModule],
	templateUrl: './admin-tab-unpaid-delivery.component.html',
})
export class AdminTabUnpaidDeliveryComponent implements OnInit, OnChanges {
	...
	loadOrders(): void {
		...
		this.orderService.getUnpaidDeliveredOrders().subscribe({
			next: (data: Order[]) => {
				this.orders = data ?? [];
				this.loading = false;
				this.cdr.markForCheck();
			...
			error: () => {
				this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to load unpaid delivery orders' });
				this.loading = false;
				this.cdr.markForCheck();
			},
		});
