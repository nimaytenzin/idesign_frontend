...
@Component({
	selector: 'app-admin-tab-to-track',
	standalone: true,
	imports: [CommonModule, FormsModule, TableModule, TagModule],
	templateUrl: './admin-tab-to-track.component.html',
})
export class AdminTabToTrackComponent implements OnInit, OnChanges {
	...
	loadOrders(): void {
		...
		this.orderService.getOrdersToTrack().subscribe({
			...
			error: () => {
				this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to load orders to track' });
				this.loading = false;
				this.cdr.markForCheck();
			},
		});
